...
###**might only want certain columns with numbers not words??**
...
Matched['Rate'] = Matched['S&P Domestic Long Term Issuer Credit Rating'].astype(str).map(lambda x: rateNum(x))
#print(Matched['Rate'])


#9) Ratings Frequency
allCo = Matched.loc[Matched['Name'] == 'CO']
print("Average Rating for CO's = ", allCo['Rate'].mean().round(2))


#10) Output file
Matched.to_csv('newFile.csv')
