...
def compareVals(dfWeBuilt, dfMACD, dfPRICE):
    ...
    counter=0
    for datetime in dfWeBuilt["dateAndTime"]:
        if  str(datetime) != str(dfPRICE["timestamp"][count]):
            print(str(datetime), str(dfPRICE["timestamp"][count]))
            #print("Missing time:", dfPRICE["timestamp"][count])
            dfPRICE=insertRow(dfPRICE, count, str(datetime)) #add the missing times into the dataframe
        counter=counter+1
...
def combineDFs(dfWeBuilt, dfImported, stock):
    ...
    priceCol="{}_GRADIENT".format(stock)
